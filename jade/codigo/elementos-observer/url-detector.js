...
window.urlDetector = {
  ...
  async extractUrlFromChat() {
    ...
    // Generar nomenclatura base (solo DD-MM-ID, sin letra de campaña)
    ...
    // Construir nomenclatura final (solo DD-MM-ID + ! si cargó)
    const nomenclaturaFinal = clienteCargo ? `${nomenclaturaBase}!` : nomenclaturaBase;
    
    // Obtener URLs únicas para evitar duplicados
    const urlsUnicas = new Map();
    for (const urlItem of urlsDeHoy) {
      if (!urlsUnicas.has(urlItem.url)) {
    ...
    // Log simplificado
    const estadoCarga = clienteCargo ? '✅ CARGÓ' : '⏳ Pendiente';
    console.log(`🏷️ ${nomenclaturaFinal} | ${urlsDeHoy.length} URL(s) de hoy | ${estadoCarga}`);
